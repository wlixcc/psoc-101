...
/* This definition is intended to prevent unexpected linker error.
   For more details please see the IAR Technical Note 49981 */
    ...
    extern void CapSense_1_EnableSensor(uint32 sensor);
    extern void CapSense_1_DisableSensor(uint32 sensor);
#endif /* (__ICCARM__) */

/* SmartSense functions */
#if (CapSense_1_TUNING_METHOD == CapSense_1__TUNING_AUTO)
    uint8 CapSense_1_lowLevelTuningDone = 0u;
    uint8 CapSense_1_scanSpeedTbl[((CapSense_1_TOTAL_SENSOR_COUNT - 1u) / 8u) + 1u];
...
uint8 CapSense_1_unscannedSnsDriveMode[CapSense_1_PORT_PIN_CONFIG_TBL_ZISE];
...
reg32* const CapSense_1_prtSelTbl[CapSense_1_CFG_REG_TBL_SIZE] = {
...
reg32* const CapSense_1_prtCfgTbl[CapSense_1_CFG_REG_TBL_SIZE] = {
...
reg32* const CapSense_1_prtDRTbl[CapSense_1_CFG_REG_TBL_SIZE] = {
...
void CapSense_1_Init(void)
{
    ...
    {
        CapSense_1_SetUnscannedSensorState(curSensor, CapSense_1_CONNECT_INACTIVE_SNS);
    }
...
void CapSense_1_Start(void)
{
    ...
    /* Initialize Advanced Centroid */
        ...
        CapSense_1_AdvancedCentroidInit();
...
void CapSense_1_DisableSensor(uint32 sensor)
{
    ...
    *CapSense_1_prtSelTbl[port] &= ~(CapSense_1_CSD_HSIOM_MASK << pinHSIOMShift);
    ...
    if(sensorState != (uint32)CapSense_1__SHIELD)
    {
        ...
        if(sensorState == (uint32)CapSense_1__SHIELD)
        {
            sensorState = (uint32)CapSense_1__GROUND;
        }
    #endif /* (CapSense_1_IS_SHIELD_ENABLE) */
        if(sensorState != (uint32)CapSense_1__HIZ_ANALOG)
        {
            /* Connected to Ground */
            inactiveConnect = (uint32)CapSense_1_SNS_GROUND_CONNECT;
        }
...
void CapSense_1_SetDriveModeAllPins(uint32 driveMode)
{
    ...
    for(curSensor = 0u; curSensor < CapSense_1_TOTAL_SENSOR_COUNT; curSensor++)
    {
        #if (CapSense_1_IS_COMPLEX_SCANSLOTS)
            /* Read sensor type: single or complex */
            snsType = CapSense_1_portTable[curSensor];
            ...
            else
            {
                ...
                snsType &= ~CapSense_1_COMPLEX_SS_FLAG;
                snsNumber = CapSense_1_indexTable[snsType];
            }

            prtNumberTmp = CapSense_1_portTable[snsNumber];
            pinNumberTmp = CapSense_1_pinShiftTbl[snsNumber];
        #else
...
void CapSense_1_PreScan(uint32 sensor)
{
    ...
    #if ((CapSense_1_TUNING_METHOD == CapSense_1__TUNING_AUTO) &&\
         (0 != CapSense_1_IS_OVERSAMPLING_EN))
        oversamplingFactor = CapSense_1_GetBitValue(CapSense_1_scanSpeedTbl, sensor);
    ...
    /* `#START CapSense_1_PreSettlingDelay_Debug` */

    /* `#END` */

#ifdef CapSense_1_PRE_SCAN_PRE_SETTLING_DELAY_DEBUG_CALLBACK
    CapSense_1_PreScan_Pre_SettlingDelay_Debug_Callback();
#endif /* CapSense_1_PRE_SCAN_PRE_SETTLING_DELAY_DEBUG_CALLBACK */
...
#ifdef CapSense_1_PRE_SCAN_DEBUG_CALLBACK
    CapSense_1_PreScan_Debug_Callback();
#endif /* CapSense_1_PRE_SCAN_DEBUG_CALLBACK */
...
#ifdef CapSense_1_POST_SCAN_DEBUG_CALLBACK
    CapSense_1_PostScan_Debug_Callback();
#endif /* CapSense_1_POST_SCAN_DEBUG_Callback */
...
void CapSense_1_SetUnscannedSensorState(uint32 sensor, uint32 sensorState)
{
        ...
        /* Read sensor type: single or complex */
        snsType = CapSense_1_portTable[sensor];

        /* Check if sensor is complex */
        if(0u != (snsType & CapSense_1_COMPLEX_SS_FLAG))
        {
            snsType &= (uint8)~CapSense_1_COMPLEX_SS_FLAG;
            snsNumber = (uint16)CapSense_1_maskTable[sensor];

            for (curSensor=0u; curSensor < snsNumber; curSensor++)
            {
                sensor = CapSense_1_indexTable[snsType + curSensor];
